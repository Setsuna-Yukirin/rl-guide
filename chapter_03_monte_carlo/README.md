...
**核心问题**：MDP 未知时，如何**从经验中学习**？

**核心思想**：
> 与其用模型计算期望，不如用采样近似期望。

**两种 MC 方法**：
- **第一访问 MC**：只用第一次访问的回报（理论简单）
- **每次访问 MC**：用所有访问的回报（数据利用高）

**核心公式**：
- MC 估计：$V(s) \approx \frac{1}{N}\sum_{i=1}^{N} G_i$
...
**2. 第一访问 MC 和每次访问 MC 的区别是什么？**

<details>
<summary>点击查看答案</summary>

**答案**：
- **第一访问 MC**：每个 episode 中，只记录第一次访问状态的回报
- **每次访问 MC**：记录每次访问状态的回报

第一访问理论分析简单，每次访问数据利用率高。两者都收敛到 V_π。
</details>

---

**3. ε-greedy 策略中 ε 的作用是什么？**
...
重要性比率 ρ = P(τ|π) / P(τ|b) 是概率的比值。

如果某个动作在π下概率高（0.9），在 b 下概率低（0.1）：
ρ = 0.9 / 0.1 = 9

长 episode 中多个比率相乘，权重可能指数级增长，导致方差极大。
</details>
...
**提示**：
1. ε_k = 1/k，k 是 episode 编号
2. 记录每个 episode 的胜率
3. 对比固定 ε 的表现

GLIE 理论上收敛到最优，但实际中可能固定 ε 足够。
</details>
